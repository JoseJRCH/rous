...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Personas como tú, tan detallistas, sensibles y perseverantes, merecen un cumpleaños lleno de cariño y sorpresas bonitas; porque en cada paso combinas esfuerzo y ternura, y este nuevo año te traerá la recompensa de todo lo que vienes construyendo.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con Cariño, José";
  signature.classList.add('visible');

  // 👉 Después de mostrar la firma, mostramos el botón
  setTimeout(() => {
    const goButton = document.getElementById("go-button");
    if (goButton) goButton.classList.add("show");
  }, 800); // retardo opcional para que aparezca suave después de la firma
}
